...
exports.GetTickets = function (req, res, next)
{
    if(req.user && req.user.role == 'employee')
    {
        ...
        if(data['search']['date'])
        {
            ...
            data['search']['date_end'] = {"$gte":date_start, "$lt": date_end};
            delete data['search']['date'];
        }

        if(!data['search']['status'])
        {
            data['search']['$or'] = [{status:'VENDIDO'}, {status:'RESERVADO'}];
        }

        if(data['search']['code'])
        {
            data['search'] = {code:data['search']['code']};
        }
        ...
        if(data['search']['employee'])
        {
            return userDB.findOne({'username': data['search']['employee']}, {'_id':1}, function(err, user){
                ...
                ticketsDB
                .find(!data['search'] ? {} : data['search'], {code:1, employee:1, print:1, status:1, date:1}, {limit:data['limit'], skip:data['offset']})
                .sort(data['order'])
                .execFind(function (err, tickets){            
                    ...
                    ticketsDB.count(!data['search'] ? {} : data['search'],function (err, count) {
    ...
    if(req.user && req.user.role == 'employee')
    {
        ...
        .find(search, function (err, tickets) {            
            ...
            .find({},{username:1,_id:1}, function (err, employees) {       
                ...
                summary = {employee:'', ordered:0, returned:0, booked:0, cancelled:0, total:0, total_tickets:0};

                for(i=0;i<employees.length;i++)
                {
                    //box.push({employee:employees[i], ordered:0, returned:0, booked:0, cancelled:0, total:0});
                    temp_box[employees[i]['_id']] = {employee:employees[i]['username'], ordered:0, returned:0, booked:0, cancelled:0, total:0, total_tickets:0};
                }
                ...
                for(i=0;i<tickets.length;i++)
                {
                    ...
                    {
                        temp_box[tickets[i]['employee_id']].returned++;
                        temp_box[tickets[i]['employee_id']].total += tickets[i]['overview']['total'];
                        summary.total += tickets[i]['overview']['total'];
                        summary.returned++;
                    }
